...
| # | Question | Blocks |
|---|---|---|
| **116** | **`item_barcode` is referenced twice by D19 and defined by no adopted decision.** Same situation `device` was in before D27. The inbound research called it the single most load-bearing undefined table, because it blocks carton receiving and every scan that resolves a barcode to an item. | The first migration, and all scanning |
...
Four blockers, and one of them (116) had been sitting in a research document since
the inbound pass without being carried across. That is the failure this register
