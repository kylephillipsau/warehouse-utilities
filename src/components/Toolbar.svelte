...
<header
    id="header"
    class="sticky top-0 z-10 flex flex-col gap-[0.55rem] bg-paper text-ink border-b-[3px] border-ink px-4 py-[0.6rem] transition-shadow"
    style:box-shadow={scrolled ? '0 4px 14px rgba(45,58,46,0.3)' : 'none'}
>
    <!-- Tier 1: app bar -->
                ...
                <div class="absolute right-0 top-[calc(100%+6px)] z-[25] flex w-[min(24rem,calc(100vw-2rem))] flex-col gap-[0.85rem] rounded-lg border-2 border-ink bg-paper p-4 shadow-[0_12px_28px_rgba(45,58,46,0.35)] max-sm:inset-x-0 max-sm:w-auto" role="dialog" aria-label="Label setup">
                    ...
                    <div class="flex flex-col gap-[0.35rem]">
                        ...
                        <div class="group-row">
                            ...
                            <span class="text-[0.85rem] text-ink/70">label{store.divisions === 1 ? '' : 's'} per page (each fills the width)</span>
                        </div>
                    </div>
